...
const float MaxLumaDiff = 0.20;

#define MAX_ANGLE_COUNT 90
#define RadiusScale 1.0
const float GlobRadiusOuterPx = 20.0 * RadiusScale;
const float GlobRadiusMidPx = 10.0 * RadiusScale;
const float GlobRadiusInnerPx = 5.0 * RadiusScale;
const int MaxSequence = MAX_ANGLE_COUNT/10;			//	thicker lines and more angles need to allow more in a sequence
...
vec3 GetGlobColour(float GlobTypef)
{
	...
	if ( GlobShowOnly != 0 && GlobType != GlobShowOnly )
		GlobType = GLOB_TOO_NOISY;
	
	if ( GlobType == GLOB_TOO_NOISY )	return vec3(0,0,0);
...
vec4 CalculateGlobFeature(vec2 uv,sampler2D ColourSource,out vec3 FeatureRays[MAX_ANGLE_COUNT])
{
	...
	//	now calc score
	//	count how many independent matches we get
	//	independent match is 1 or 2 matches in sequence
	int Independents = 0;
	int Blocks = 0;
	int MatchCounter = 0;
	int LastIndepdent = 0;
